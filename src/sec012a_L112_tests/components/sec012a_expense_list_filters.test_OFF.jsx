...
import { ExpenseListFilters } from "../../sec009a_components/sec011a_L103_expense_list_filters.jsx";

import { default_filters, filters } from '../fixtures/sec012a_DATA_filters.js';
...
describe ("COMPONENT TEST FOR FILTERS - sec011a_L103_expense_list_filters.jsx", () =>  {
    ...
    const GC_pram_default = new CLS_prams (default_filters);
    ...
    it ('should render CLS_expense_list_filters with supplied data correctly', () =>  {
        const prams = new CLS_prams (filters);
        expect(prams.G_wrapper).toMatchSnapshot();
    } );

    it ('should text change correctly', () =>  {
        const prams = new CLS_prams (filters);
        const value = 'bill';
        prams.G_wrapper.find('#TBX_filters').simulate('change', { target: {value} });
        expect(prams.G_set_text_filter).toHaveBeenLastCalledWith(value);
    } );

    it ('should sort expenses by date', () =>  {
        const prams = new CLS_prams (filters);
        const value = 'date';
        prams.G_wrapper.find('select').simulate('change', { target: {value}});
        //expect(prams.G_sort_by).toHaveBeenLastCalledWith(value);
        expect(prams.G_sort_by_date).toHaveBeenCalled();
    } );
    ...
    it ('should sort expenses by amount', () =>  {
        ...
        prams.G_wrapper.find('select').simulate('change', { target: {value} } );
        //expect(prams.G_sort_by).toHaveBeenLastCalledWith(value);
        expect(prams.G_sort_by_amount).toHaveBeenCalled();
    } );
    ...
    it ('should handle date changes', () =>  {
        ...
        prams.G_wrapper.find('withStyles(DateRangePicker)')
                           .prop('onDatesChange') ({ startDate, endDate });

        expect(prams.G_set_start_date).toHaveBeenLastCalledWith(startDate);
        expect(prams.G_set_end_date).toHaveBeenLastCalledWith(endDate);
    } );
    ...
    it ('should handle date focus changes', () =>  {
        ...
        prams.G_wrapper.find('withStyles(DateRangePicker)')
        //prams.G_wrapper.find('DateRangePicker')
                           .prop('onFocusChange') (calendarFocused);

        expect(prams.G_wrapper.state('calendarFocused')).toBe(calendarFocused);
    } );
